...
@pytest.mark.asyncio
async def test_run_agent_uses_ephemeral_context_without_mutating_user_message(monkeypatch, tmp_path):
    _install_fake_agent(monkeypatch)
    runner = _make_runner()
    ...
    result = await runner._run_agent(
        ...
        session_id="session-1",
        session_key="agent:main:discord:thread:12345",
        ephemeral_context="[Recent Discord channel context]\n- Ramona: hello",
    )

    assert result["final_response"] == "ok"
    assert _CapturingAgent.last_user_message == "hi"
    assert _CapturingAgent.last_init["ephemeral_system_prompt"] == (
